---
layout: default
title: gcc、clang编译相关知识
---
# gcc、clang编译相关知识
## gcc的attribute特性
开源的程序中常看到一些宏定义，如：`__GNUC__`，`__ELF__`等等。却看不到内置宏定义是在哪些地方定义的，其实内置宏定义可以在终端输入如下命令：`gcc -E -dM - </dev/null``查看，mac10.12上的显示的所有内置宏定义
...
#define __x86_64__ 1
```
在xcode中经常使用`__block`,`__weak`等宏来控制block块访问的变量及防止循环引用。但`__block`及`__weak`是什么呢？在xcode代码块写下如下代码：
...
```
通过使用`xcrun -sdk iphoneos clang -E ViewController.m`对宏定义展开
...
## clang编译framework
在终端中使用`clang`编译代码的命令如下`xcrun -sdk iphoneos clang -rewrite-objc -arch armv7s -mios-version-min=6.1 viewcontroller.m`
`xcrun -sdk iphoneos clang -rewrite-objc -arch armv7s  -fobjc-arc -stdlib=libc++ -mmacosx-version-min=10.7 -fobjc-runtime=macosx-10.7 -Wno-deprecated-declarations MLDemo/ViewController.m`
